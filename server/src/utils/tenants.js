...
function isTenantActive(tenantRow) {
  ...
  if (status !== "active") {
    return {
      ...
      error: "Подписка приостановлена. Обратитесь к ответственному администратору.",
    };
  }
  ...
  if (!expiresAt || Number.isNaN(expiresAt.getTime())) {
    return {
      ...
      error: "Подписка настроена некорректно. Обратитесь к ответственному администратору.",
    };
  }
  ...
  if (expiresAt.getTime() < Date.now()) {
    return {
      ...
      error:
        "Срок подписки истёк. Доступ будет открыт после подтверждения оплаты ответственным администратором.",
    };
  }
